...
import * as seatingApi from '@/lib/api/seating'
...
export default function SeatingPage() {
  ...
  useEffect(() => {
    if (!eventId) return
    setLoading(true)
    seatingApi.listTables(eventId).then(setTables).finally(() => setLoading(false))
  }, [eventId])

  const assignedIds = useMemo(() => new Set(tables.flatMap(t => t.guests)), [tables])
  const unassigned = guests.filter(g => !assignedIds.has(g.id))

  const addTable = async () => {
    if (!newTableName.trim() || !eventId) return
    try {
      const created = await seatingApi.createTable(eventId, { name: newTableName, capacity: newTableCapacity, shape: 'round' })
  ...
  const removeGuest = async (tableId: string, guestId: string) => {
    if (!eventId) return
    try {
      await seatingApi.assignGuest(eventId, { guestId, tableId: null })
      setTables(await seatingApi.listTables(eventId))
    } catch (err) {
      toast.error(err instanceof ApiError ? err.message : 'Une erreur est survenue')
    }
  }
  ...
  const assignGuestToTable = async (tableId: string, guestId: string) => {
    if (!eventId) return
    try {
      await seatingApi.assignGuest(eventId, { guestId, tableId })
      setTables(await seatingApi.listTables(eventId))
      setAssignModal(null)
    ...
    <>
      ...
      <Modal open={addTableModal} onClose={() => setAddTableModal(false)} title="Ajouter une table" size="sm"
        ...
        <div className="space-y-4">
          ...
          <Input label="Capacité" type="number" value={newTableCapacity} onChange={e => setNewTableCapacity(Number(e.target.value) || 1)} />
        </div>
      </Modal>
        ...
        <Modal open onClose={() => setAssignModal(null)} title="Assigner un invité" size="sm">
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {unassigned.length === 0 ? (
              ...
              <button
                key={guest.id}
                onClick={() => assignGuestToTable(assignModal.tableId, guest.id)}
